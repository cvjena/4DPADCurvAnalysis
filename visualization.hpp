...
public:
  ...
  void show(rs2::frame frame) { show(frame, {0, 0, (float)_width, (float)_height}); }
...
struct glfw_state {
  glfw_state() : yaw(15.0), pitch(15.0), last_x(0.0), last_y(0.0), ml(false), offset_x(2.f), offset_y(2.f), tex() {}
  double yaw;
  double pitch;
  double last_x;
  double last_y;
